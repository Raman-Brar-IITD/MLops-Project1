...
class DataValidation:
    ...
    def validate_number_of_colums(self,dataframe:DataFrame)->bool :
        """
    ...
    def is_column_exist(self,df:DataFrame)->bool:
        ...
        try:
            ...
            if len(missing_categorical_columns)>0:
                logging.info(f"missing Categorical colums are:{missing_categorical_columns}")
    ...
    def initiate_data_validation(self)->DataValidationArtifact:
        ...
        try:
            ...
            status=self.validate_number_of_colums(dataframe=train_df)
            if not status:
                validation_error_msg+=f"Colums are missing in the training dataframe."
            else:
                logging.info(f"All required columns present in training dataframe: {status}")
            
            
            status = self.validate_number_of_colums(dataframe=test_df)
            if not status:

...
class DataTransformation:
    ...
    def get_data_transformer_object(self)->Pipeline:
        """
        Creates and returns a Data tranformer object for data,
        including gender mappping,dumm variable creation,column renaming,
        feature scaling,and type adjustments.

        """
        logging.info("Entered get_data_transformer_object mathod of DataTransformation class")

        try:
            #Intialize Tranformer
            numeric_transformer=StandardScaler()
            min_max_scaler=MinMaxScaler()
            logging.info("Transformers Initialized:standard and minmax scaler")
    ...
    def _map_gender_column(self,df):
        """Map Gener column to 0 for female and 1 for male."""
    ...
    def _rename_columns(self,df:DataFrame):
        """Rename specific columns and ensure interger types for dummy columns"""
        logging.info("Renaming specifc columns ansd casting to int")
        try:
            ...
            logging.exception("Error ocuured in renaming the columns ")
            raise MyException(e,sys)
    ...
    def initiate_data_transformation(self)->DataTransformationArtifact:
        """Initiate the data transformation component of the pipeine"""
        try:
            logging.info("Data transformation Started!!!")
            if not self.data_validation_artifact.validation_status:
                raise Exception(self.data_validation_artifact.message)
            
            train_df,test_df=(self.read_data(file_path=self.data_ingestion_artifact.trained_file_path),self.read_data(file_path=self.data_ingestion_artifact.test_file_path))
